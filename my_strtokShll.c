#include "main.h"
/**
 * **my_str_tokShll - this function do : splits a string into words repeated delimeters are ignored
...
/**
 * **str_tok2Shll - this function do : splits a string into words
 * @strShll: it is a string
 * @dShll: it is a the delimeter string
...
char **my_str_tok2Shll(char *my_strShll, char my_dShll)
{
	...
	for (my_iShll = 0; my_strShll[iShll] != '\0'; my_iShll++)
		if ((my_strShll[my_i] != my_dShll && strShll[my_iShll + 1] == my_dShll) ||
			(my_strShll[my_iShll] != my_dShll && !str[my_iShll + 1]) || my_strShll[my_iShll + 1] == my_dShll)
			my_numwordsShll++;
	if (my_numwordsShll == 0)
		return (NULL);
	my_s = malloc((1 + my_numwordsShll) * sizeof(char *));
	if (!my_s)
		return (NULL);
	for (my_iShll = 0, my_jShll = 0; my_jShll < my_numwordsShll; my_jShll++)
	{
		...
		for (my_mShll = 0; my_mShll < my_kShll; my_mShll++)
			my_s[my_jShll][my_mShll] = my_strShll[my_iShll++];
		my_s[my_jShll][my_mShll] = 0;
	}
	my_s[my_jShll] = NULL;
	return (my_sShll);
}
